...
class FabSplitterOperation(PostProcessOperation):
    ...
    def execute(self, context: PostProcessContext) -> None:
        ...
        try:
            ...
            util_count, floor_count, no_level_count, other_count = process_stage(
                stage=stage,
            ...
            context.on_info(
                _TAG,
                f"완료: UTIL {util_count}개 + 층별 {floor_count}개 + NO_LEVEL {no_level_count}개 + OTHER {other_count}개 파일 생성"
            )
        finally:
